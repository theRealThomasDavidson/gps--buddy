...
- Concrete implementations (MapLibre, Valhalla, ORS, etc.) live behind **interfaces**.
...
2. **Next direction while moving** — Advance beyond the static provider step list: pick **current / next** maneuver from position along the route + step geometries, with UI that highlights the upcoming turn (voice is out of scope until this is solid).
